...
export function buildAntdTheme(
  ...
  return {
    ...
    token: {
      ...
      // No borders anywhere — surfaces separate by background contrast instead, which reads
      // harder/flatter than a soft 1px outline. Focus/active states still show up fine since
      // they lean on `activeShadow`/`boxShadow` tokens rather than border width.
      lineWidth: 0,
      wireframe: false,
    ...
    components: {
      ...
      Button: {
        ...
        controlHeight: 30,
        borderRadius: radius.sm,
        // No border to draw the button's edge — a background fill does that instead, so
        // "default" buttons stay legible at rest instead of only appearing on hover.
        defaultBg: isDark ? p.bgElevated : p.bgSpotlight,
        defaultHoverBg: p.selectionBg,
      ...
      Input: {
        // Unlike buttons/cards/tables, search/filter fields need a real outline — without one,
        // an empty input has no way to signal "type here" against the page background.
        lineWidth: 1,
...
export function syncDocumentTheme(
  ...
  const vars: Record<string, string> = {
    ...
    '--ml-resource-sider-width': '260px',
    '--ml-cluster-tab-width': '220px',
    '--ml-glass-bg': withAlpha(p.sidebarBg, isDark ? 0.88 : 0.92),
    '--ml-glass-border': p.sidebarDivider,
    '--ml-status-success-bg': withAlpha(p.success, isDark ? 0.14 : 0.1),
    '--ml-status-success-fg': p.success,
    '--ml-status-warning-bg': withAlpha(p.warning, isDark ? 0.14 : 0.1),
    '--ml-status-warning-fg': p.warning,
    '--ml-status-danger-bg': withAlpha(p.error, isDark ? 0.14 : 0.1),
    '--ml-status-danger-fg': p.error,
    '--ml-status-info-bg': withAlpha(p.info, isDark ? 0.14 : 0.1),
    '--ml-status-info-fg': p.info,
