...
import { demoCrdKinds, demoDiscovery, demoDynamicList, isDemoCluster } from '../k8s/demoMode'
...
export function registerDiscoveryHandlers(): void {
  ipcMain.handle(IPC.DISCOVERY_LIST, async (_e, req: DiscoveryRequest): Promise<DiscoveryResponse> => {
    if (isDemoCluster(req.clusterId)) return demoDiscovery(req.clusterId)
  ...
  ipcMain.handle(
    ...
    async (_e, req: CustomResourceKindsRequest): Promise<CustomResourceKindsResponse> => {
      if (isDemoCluster(req.clusterId)) return { kinds: demoCrdKinds(req.clusterId) }
      try {
        const clients = clusterManager.require(req.clusterId)
  ...
  ipcMain.handle(
    ...
    async (_e, req: DynamicResourceListRequest): Promise<DynamicResourceListResponse> => {
      if (isDemoCluster(req.clusterId)) return { items: demoDynamicList(req.kind, req.namespace, req.clusterId) }
      try {
        const clients = clusterManager.require(req.clusterId)
